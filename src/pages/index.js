...
const BlogIndex = ({ data, location }) => {
  ...
  let items = group.map(tag => (
    ...
    :tag.fieldValue === "motorcycles" ?
      (
        /* bolt.svg ( https://tablericons.com/ ) */
        <div>
          <Link to={`/tags/${kebabCase(tag.fieldValue)}/`} style={{ boxShadow: 'none' }}>
            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-bike" width="44" height="44" viewBox="0 0 24 24" stroke-width="1.5" stroke="#2196F3" fill="none" stroke-linecap="round" stroke-linejoin="round">
              <path stroke="none" d="M0 0h24v24H0z"/>
         ...
         <p style={{fontSize: '0.9em', fontWeight: 'bold'}}>{"Motorcycles"} ({tag.totalCount})</p>
        </div>
      )
